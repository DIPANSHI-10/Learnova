...
import { buildFlashcards, buildQuiz, buildStudyPlan, coachReply, createSummary } from "../services/aiService";
...
export const learningRouter = router({
  chat: router({
    ...
    send: protectedProcedure.input(z.object({ message: z.string().trim().min(1).max(4000), attachmentText: z.string().trim().max(10000).optional(), conversationId: z.string().trim().min(1).max(100).default("primary") })).mutation(async ({ ctx, input }) => {
      const db = await dbOrThrow();
      ...
      const enrichedMessage = input.attachmentText ? `${input.message}\n\nAttached study material:\n${input.attachmentText.slice(0, 10000)}` : input.message;
  ...
  quiz: router({
    ...
    generate: protectedProcedure.input(z.object({ topic: z.string().trim().min(1).max(180), difficulty: z.enum(["easy", "medium", "hard"]), questionType: z.enum(["mcq", "true_false"]), count: z.number().int().min(5).max(20) })).mutation(async ({ ctx, input }) => {
      const db = await dbOrThrow();
      const questions = buildQuiz(input.topic, input.difficulty, input.count, input.questionType);
  ...
  flashcards: router({
    ...
    generate: protectedProcedure.input(z.object({ topic: z.string().trim().min(1).max(180), source: z.string().trim().max(12000).optional() })).mutation(async ({ ctx, input }) => {
      const db = await dbOrThrow();
      const cards = buildFlashcards(input.topic, input.source);
